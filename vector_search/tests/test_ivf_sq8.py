import sys
from vector_search.src.ivf import IVF_SQ8
...
#     ivf = IVF_SQ8(identity_data[0], n_clusters=128, topk=10)
...
# def test_proxy_cosine_distance():
#     X = np.array([[1, 0, 0], [0, 1, 0]])
#     Y = np.array([[1, 0, 0], [0, 1, 0]])
#     ivf = IVF_SQ8(X, topk=3, n_clusters=2)
#     distances = ivf._compute_proxy_cosine_distance(X, Y)
...
def test_ivf_flat_exact_search(n_clusters):
    ...
    base = base/np.linalg.norm(base, axis=-1)[:, np.newaxis]
    n_query_idxes = [123]#np.random.choice(base_size, query_size, replace=False)
    query = base[n_query_idxes]
    
    ivf = IVF_SQ8(base, n_clusters=n_clusters, topk=1, n_probes=n_clusters)
...
#     ivf = IVF_SQ8(base, topk=1, n_clusters=n_clusters, n_probes=n_probes)
